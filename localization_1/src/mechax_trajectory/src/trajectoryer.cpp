...
Trajectoryer::Trajectoryer(const rclcpp::NodeOptions & options) : Node("trajectory", options)
{
    ...
    power_rune_sub_ = this->create_subscription<geometry_msgs::msg::PointStamped>(
        ...
        //辅瞄                            
    outpost_sub = this->create_subscription<rm_msgs::msg::AssistData>(
        "/assistaim_point", 10, std::bind(&Trajectoryer::outpostfumiao_callback, this, std::placeholders::_1));
...
//这个target是在哪个坐标系下的？？is_repeat是什么信息？car_vx,armor_vx,v_yaw有什么区别
...
//前哨站辅瞄回调
void Trajectoryer::outpostfumiao_callback(rm_msgs::msg::AssistData msg){
        two_resistance_model(
            msg.rel_x,
            msg.rel_y,
            msg.rel_z,
    ...
    angle_yaw = atan2(msg.rel_y, msg.rel_z);
